...
module.exports.run = async (retryAuto = false) => {
  ...
  if (mode === 'libre' && speedtest.server) {
    let server = Object.entries(serverController.getLibreServers()).filter(
      ([, value]) => value === speedtest.server.name,
    )[0][0];
  ...
  if (Object.keys(speedtest).length <= 1)
    throw { message: 'No response, even after trying again, test timed out.' };
...
module.exports.create = async (type = 'auto', retried = false) => {
  ...
  try {
    ...
    let { ping, download, upload, time, resultId } = await parseData.parseData(
      process.env.PREVIEW_MODE === 'true' ? 'ookla' : mode,
      test,
    );

    let testResult = await tests.create(
      ping,
      download,
      upload,
      time,
    ...
    console.log(
      `Test #${testResult} was executed successfully in ${time}s. 🏓 ${ping} ⬇ ${download}️ ⬆ ${upload}️`,
    );
    ...
    console.log(
      `Test #${testResult} was not executed successfully. Please try reconnecting to the internet or restarting the software: ` +
        e.message,
    );
  }
};
