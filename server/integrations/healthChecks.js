...
const sendPing = async (url, path, error, triggerActivity) => {
  ...
  await axios
    .post(url, error, { headers: { 'user-agent': 'MySpeed/HealthAgent' } })
...
module.exports = (registerEvent) => {
  registerEvent('minutePassed', async (integration, data, triggerActivity) => {
    if (integration.data.url)
      await sendPing(integration.data.url, undefined, undefined, triggerActivity);
  });
  ...
  return {
    ...
    fields: [{ name: 'url', type: 'text', required: true, regex: /https?:\/\/.+/ }],
  };
};
