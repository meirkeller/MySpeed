...
app.put('/:integrationName', password(false), async (req, res) => {
  ...
  if (process.env.PREVIEW_MODE === 'true')
    return res
      .status(403)
      .json({ message: "For security reasons, you can't create integrations in preview mode" });
...
app.patch('/:id', password(false), async (req, res) => {
  ...
  if (process.env.PREVIEW_MODE === 'true')
    return res
      .status(403)
      .json({ message: "For security reasons, you can't update integrations in preview mode" });
...
app.delete('/:id', password(false), async (req, res) => {
  if (process.env.PREVIEW_MODE === 'true')
    return res
      .status(403)
      .json({ message: "For security reasons, you can't delete integrations in preview mode" });
