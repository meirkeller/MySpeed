...
app.put('/', password(false), async (req, res) => {
  if (process.env.PREVIEW_MODE === 'true')
    return res
      .status(403)
      .json({ message: "For security reasons, you can't create nodes in preview mode" });

  if (!req.body.name || !req.body.url)
    return res.status(400).json({ message: 'Missing parameters', type: 'MISSING_PARAMETERS' });
  ...
  nodes.checkStatus(url, req.body.password).then(async (result) => {
    ...
    res.json({
      id: (await nodes.create(req.body.name, url, req.body.password)).id,
      type: 'NODE_CREATED',
    });
  });
});
...
app.delete('/:nodeId', password(false), async (req, res) => {
  if (process.env.PREVIEW_MODE === 'true')
    return res
      .status(403)
      .json({ message: "For security reasons, you can't delete nodes in preview mode" });
...
app.patch('/:nodeId/name', password(false), async (req, res) => {
  if (process.env.PREVIEW_MODE === 'true')
    return res
      .status(403)
      .json({ message: "For security reasons, you can't update nodes in preview mode" });
...
app.patch('/:nodeId/password', password(false), async (req, res) => {
  ...
  nodes.checkStatus(node.url, req.body.password).then(async (result) => {
    ...
    await nodes.updatePassword(
      req.params.nodeId,
      req.body.password === 'none' ? null : req.body.password,
    );
