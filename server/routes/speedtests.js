...
app.get('/', password(true), async (req, res) => {
  if (req.query.limit && /[^0-9]/.test(req.query.limit))
    return res
      .status(400)
      .json({ message: 'You need to provide a correct number in the limit parameter' });

  if (req.query.afterId && /[^0-9]/.test(req.query.afterId))
    return res
      .status(400)
      .json({ message: 'You need to provide a correct number in the afterId parameter' });
...
app.post('/run', password(false), async (req, res) => {
  if (pauseController.currentState)
    return res.status(410).json({ message: 'The speedtests are currently paused' });
  if ((await config.getValue('provider')) === 'none')
    return res.status(410).json({ message: 'No provider selected' });
  let speedtest = await testTask.create('custom');
  if (speedtest !== undefined)
