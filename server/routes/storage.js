...
app.delete('/tests/history', password(false), async (req, res) => {
  ...
  res
    ...
    .json({ message: result ? 'Tests deleted' : 'Error deleting tests' });
});
...
app.put('/tests/history', password(false), async (req, res) => {
  ...
  res
    .status(result ? 200 : 500)
    .json({ message: result ? 'Tests imported' : 'Error importing tests' });
});
...
app.put('/config', password(false), async (req, res) => {
  ...
  res
    .status(result ? 200 : 500)
    .json({ message: result ? 'Config imported' : 'Error importing config' });
});
...
app.delete('/config', password(false), async (req, res) => {
  ...
  res
    .status(result ? 200 : 500)
    .json({ message: result ? 'Config reset' : 'Error resetting config' });
});
