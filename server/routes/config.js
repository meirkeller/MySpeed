...
app.get('/', password(true), async (req, res) => {
  ...
  (await config.listAll()).forEach((row) => {
    if (
      row.key !== 'password' &&
      !(req.viewMode && ['ooklaId', 'libreId', 'cron', 'passwordLevel'].includes(row.key))
    )
  ...
  if (process.env.PREVIEW_MODE === 'true')
    configValues['previewMessage'] = String(
      process.env.PREVIEW_MESSAGE || 'The owner of this instance has not provided a message',
    );

  if (Object.keys(configValues).length === 0)
    return res.status(404).json({ message: 'Hmm. There are no config values. Weird...' });
