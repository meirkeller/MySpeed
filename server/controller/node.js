...
module.exports.create = async (name, url, password) =>
  await nodes.create({ name: name, url: url, password: password });
...
module.exports.updateName = async (nodeId, name) =>
  await nodes.update({ name: name }, { where: { id: nodeId } });

module.exports.updatePassword = async (nodeId, password) =>
  await nodes.update({ password: password }, { where: { id: nodeId } });

module.exports.checkStatus = async (url, password) => {
  if (password === 'none') password = undefined;
  const api = await axios
    .get(url + '/api/config', { headers: { password: password } })
    .catch(() => {
      return 'INVALID_URL';
    });
...
module.exports.proxyRequest = async (url, req, res) => {
  ...
  if (response === 'INVALID_URL') return res.status(500).json({ message: 'Internal server error' });
