...
const triggerActivity = async (id, error = false) => {
  await IntegrationData.update(
    { lastActivity: new Date().toISOString(), activityFailed: error },
    { where: { id: id } },
  );
};
...
module.exports.triggerEvent = async (name, data) => {
  ...
  for (const module of events[name]) {
    ...
    for (const integration of active)
      await module.callback(integration, data, (error = false) =>
        triggerActivity(integration.id, error),
      );
  }
};

module.exports.initialize = () =>
  readdirSync(path.join(__dirname, '../integrations')).forEach(async (file) => {
    if (file.endsWith('.js')) {
      const integrationName = file.replace('.js', '');
      integrations[integrationName] = require(path.join(__dirname, '../integrations', file))(
        registerEvent(integrationName),
      );
...
module.exports.patch = async (id, data) => {
  ...
  IntegrationData.update(
    { data: { ...JSON.parse(item.data), ...data }, displayName },
    { where: { id: id } },
  );
  return true;
};
...
module.exports.getIntegrations = () => {
  ...
  for (const [name, integration] of Object.entries(integrations)) {
    ...
    updatedIntegration.fields = updatedIntegration.fields.map((field) => ({
      ...field,
      regex: field.regex ? field.regex.source : undefined,
    }));
