...
const binaryPath =
  `${binaryDirectory}/librespeed-cli` + (process.platform === 'win32' ? '.exe' : '');
...
module.exports.downloadFile = async () => {
  const binary = binaries.libreList.find(
    (b) => b.os === process.platform && b.arch === process.arch,
  );

  if (!binary)
    throw new Error(
      `Your platform (${process.platform}-${process.arch}) is not supported by the LibreSpeed CLI`,
    );

  await new Promise((resolve) => {
    file({ postfix: binary.suffix }, async (err, path) => {
      const location = await new Promise((resolve) =>
        get(downloadPath + binary.suffix, (res) => {
